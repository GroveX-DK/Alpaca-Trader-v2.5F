...
def predict_rankings() -> Tuple[str, float, List[Tuple[str, float]]]:
    """
    Returnér (bedste_symbol, score, fuld_ranking_med_forudsigelse).

    score: forudsagt næste dags open→close i procent (samme label som ved træning).

    OHLCV hentes via fetch_daily_bars (samme sti som træning). Med
    OHLCV_CACHE_ENABLED opdateres manglende interval fra Alpaca og merges i cache;
    ved tail-API-fejl kan trimmed cache bruges som fallback.
    """
    ...
    for sym in config.WATCHLIST:
        ...
        xt = xt.to(device)
        with torch.no_grad():
            pred = float(model(xt).squeeze().detach().cpu().numpy())
        score_map[sym] = pred
